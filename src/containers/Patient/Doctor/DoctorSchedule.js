...
class DoctorSchedule extends Component {
  ...
  getArrDays = (language) => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      if (language === LANGUAGES.VI) {
        if (i === 0) {
          ...
          let labelVi = moment(new Date())
            .add(i, "days")
            .format("dddd - DD/MM");
          object.label = this.stringFirstUpper(labelVi);
        }
      } else {
        if (i === 0) {
          let ddMM = moment(new Date()).format("DD/MM");
      ...
      object.value = moment(new Date()).add(i, "days").startOf("day").valueOf(); // ham tang ngay
      allDays.push(object);
    }
    return allDays;
  };
            ...
            <div className="time-content">
              {allAvalableTime && allAvalableTime.length > 0 ? (
                ...
                <div className="no-schedule">
                  <FormattedMessage id="manage-schedule.calendar-message" />
                </div>
              )}
            </div>
          </div>
